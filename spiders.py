...
class BasicSpider:
    def __init__(self, content_tups, media_tups,
        ...
        self.content_rplc_commmas = content_rplc_commas if content_rplc \
    ...
    def get_content(self, soup):
        content_txt = self.get_text(soup, self.content_tups, self.content_rplc_commmas, self.content_splitters)
        return content_txt
    ...
    def get_text(self, soup, tups, rplc_commas, splitters):
        text_wapper = None
        for tup in tups:
            text_wapper = self._parse_tup(tup, soup)
            if len(text_wapper) > 0:
                break
        if text_wapper is None:
            raise Exception('fail to find text wrapper')
        text = text_wapper[0].text
    ...
    def _parse_tup(self, tup, soup):
        text_wapper = soup.find_all(name=tup['tag'], attrs={tup['attr_name']: tup['attr']})
        if len(text_wapper) > 0 and tup.get('child', None) is not None:
            text_wapper = self._parse_tup(tup['child'], text_wapper[0])
        return text_wapper
